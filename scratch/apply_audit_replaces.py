...
file_path = r"e:\going on works\SkillSync\code\ai-engine\llm_client.py"

with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

start_marker = "    # 15. Comprehensive CV Audit (audit_analyzer.py)"
start_idx = content.find(start_marker)

# Find the end marker
# Let's match the exact unique end marker of the risk_benchmarking_gaps block
end_marker = '                "issues": risk_issues\n            }\n        })'
end_idx_sub = content.find(end_marker)

if end_idx_sub != -1:
    end_idx = end_idx_sub + len(end_marker)
else:
    end_idx = -1
...
new_code = r"""    # 15. Comprehensive CV Audit (audit_analyzer.py)
...
if start_idx != -1 and end_idx != -1:
    content_replaced = content[:start_idx] + new_code + content[end_idx:]
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(content_replaced)
    print("SUCCESS: mock audit analyzer replaced in llm_client.py")
else:
    print("ERROR: could not find markers in llm_client.py")
